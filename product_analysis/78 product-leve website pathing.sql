...
-- SELECT DISTINCT next_pageview_url FROM session_w_next_pageview_url
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN next_pageview_url IS NOT NULL THEN website_session_id ELSE NULL END) AS w_next_pg, -- NULL when user drop after seeing /product page
	...
	COUNT(DISTINCT CASE WHEN next_pageview_url = '/the-forever-love-bear' THEN website_session_id ELSE NULL END) AS to_lvoebear,
